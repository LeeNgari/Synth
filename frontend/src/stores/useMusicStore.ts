...
export const useMusicStore = create<MusicStore>((set) => ({
  ...
  fetchSongsByIds: async (songs: any[]) => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/songs/by-ids?ids=${ids.join(",")}`,
        {
          credentials: "include",
        },
      );
  ...
  deleteAlbum: async (id) => {
    set({ isLoading: true, error: null });
    try {
      await axios.delete(`http://localhost:5000/api/admin/albums/${id}`, {
        withCredentials: true,
      });
      set((state) => ({
  ...
  fetchHistorySongs: async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:5000/api/songs/history",
        {
          withCredentials: true,
        },
      );
  ...
  fetchMadeForYouSongs: async () => {
    ...
    try {
      const response = await axiosInstance.get(
        "http://localhost:5000/api/songs/made-for-you",
      );
  ...
  fetchUserPlaylists: async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:5000/api/playlists/me",
        {
          withCredentials: true,
        },
      );
